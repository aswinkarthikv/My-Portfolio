...
export const ContactSection: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://formsubmit.co/ajax/aswinkarthik1807@gmail.com', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          _subject: `⚡ New Dispatch from ${formData.name} via Cinematic Portfolio`,
          _template: 'table',
          _captcha: 'false',
        }),
      });
      ...
      if (res.ok && data.success !== 'false') {
        ...
        setError('Transmission encountered an anomaly. Please try again.');
      }
    } catch {
      setError('Network dispatch error. Please verify your connection.');
    } finally {
          ...
          >
            ...
            {sent ? (
              <div className="py-14 text-center space-y-4">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full border border-[#D4AF37] text-[#D4AF37] text-lg shadow-[0_0_15px_rgba(212,175,55,0.4)]">
                  ✓
                </div>
                ...
                <p className="text-xs text-[#A8988B] font-light max-w-sm mx-auto" style={{ fontFamily: "'Montserrat', sans-serif" }}>
                  Transmission successfully dispatched directly to <span className="text-[#F7E7C4] font-medium">aswinkarthik1807@gmail.com</span>.
                </p>
                <div className="pt-3">
                  <button
                    type="button"
                    onClick={() => setSent(false)}
                    className="px-5 py-2 border border-[#8C6D4F]/40 hover:border-[#D4AF37] text-[#E8DFD8] text-[10.5px] font-mono tracking-[0.2em] uppercase rounded-sm transition-colors"
                  >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                  ...
                  style={{ fontFamily: "'Montserrat', sans-serif" }}
                >
                  {isSubmitting ? 'DISPATCHING TRANSMISSION...' : 'EXECUTE DISPATCH ↗'}
                </button>
